...
export default function FilterBar({ filters }: FilterBarProps) {
  return (
    <div
      className="rounded-xl bg-white flex flex-wrap gap-x-6 gap-y-3 px-6 py-4 mb-6"
      style={{ border: '1px solid var(--ws-gray-200)' }}
    >
      {filters.map((filter) => (
        <div key={filter.label} className="flex items-center gap-3">
          <span className="text-sm font-medium whitespace-nowrap" style={{ color: 'var(--ws-gray-400)' }}>
            {filter.label}
          </span>
